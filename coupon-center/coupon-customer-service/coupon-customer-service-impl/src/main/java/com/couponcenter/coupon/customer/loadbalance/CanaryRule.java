package com.couponcenter.coupon.customer.loadbalance;
...
import static com.couponcenter.coupon.customer.constant.Constant.TRAFFIC_VERSION;
...
public class CanaryRule implements ReactorServiceInstanceLoadBalancer {
    ...
    Response<ServiceInstance> getInstanceResponse(List<ServiceInstance> instances, Request request) {
        // 1. 打印当前所有实例及其 metadata
        for (ServiceInstance inst : instances) {
            log.info("实例 port={} metadata={}", inst.getPort(), inst.getMetadata());
        }
        ...
        log.info("请求 header: {}", headers);

        String trafficVersion = headers.getFirst(TRAFFIC_VERSION);
        log.info("分流关键字 traffic-version={}", trafficVersion);
        ...
        if (StringUtils.isBlank(trafficVersion)) {
            ...
            log.info("没有traffic-version，参与分流实例数: {}", noneCanaryInstances.size());
